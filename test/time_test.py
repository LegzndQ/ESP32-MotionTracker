    ...
    with open("mpu6050.txt", "w") as file:
        while True:
            ...
            try:
                ...
                print(f"接收耗时: {recv_time - start_time:.6f}秒")
                print(f"解码耗时: {decode_time - recv_time:.6f}秒")
                print(f"解析耗时: {parse_time - decode_time:.6f}秒")
                print(f"更新耗时: {update_time - parse_time:.6f}秒")
                print(f"写入耗时: {write_time - update_time:.6f}秒")
                print(f"打印耗时: {print_time - write_time:.6f}秒")
